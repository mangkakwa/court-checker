...
def check_date(target_date):
        ...
        try:
                    resp = requests.post(API_URL,
                                                     headers={"Content-Type":"application/json","Cookie":"PHPSESSID="+PHPSESSID,"Referer":BASE_URL+"/booking.php","User-Agent":"Mozilla/5.0"},
                    ...
                    if resp.status_code != 200 or "login" in resp.text.lower()[:100]:
                                ...
                                for slot in resp.json():
                                                ...
                                                if court not in WATCH_COURTS or ts not in WATCH_TIMES: continue
                                                                available.append({"date":target_date,"court":court,"time":ts,"loc":slot.get("locName",""),"price":slot.get("stadiumtimePrice","0"),"key":target_date+"|"+court+"|"+ts})
        except Exception as e:
